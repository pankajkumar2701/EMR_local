...
{
    /// <summary>
    /// Controller responsible for managing contact-related operations in the API.
    /// </summary>
    /// <remarks>
    ...
    public class ContactController : ControllerBase
    {
        ...
        public ContactController(EMR_localContext context)
        {
            _context = context;
        }
        ...
        public IActionResult Get([FromQuery] string filters, string searchTerm, int pageNumber = 1, int pageSize = 10, string sortField = null, string sortOrder = "asc")
        {
            ...
            var query = _context.Contact.IncludeRelated().AsQueryable();
            int skip = (pageNumber - 1) * pageSize;
            var result = FilterService<Contact>.ApplyFilter(query, filterCriteria, searchTerm);
        ...
        {
            var entityData = _context.Contact.IncludeRelated().FirstOrDefault(entity => entity.Id == id);
        ...
        public IActionResult DeleteById([FromRoute] Guid id)
        {
            ...
            _context.Contact.Remove(entityData);
        ...
        public IActionResult UpdateById(Guid id, [FromBody] Contact updatedEntity)
        {
            ...
            this._context.Contact.Update(updatedEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
        ...
        public IActionResult UpdateById(Guid id, [FromBody] JsonPatchDocument<Contact> updatedEntity)
        {
            ...
            var existingEntity = this._context.Contact.FirstOrDefault(t => t.Id == id);
            if (existingEntity == null)
                return NotFound();
            updatedEntity.ApplyTo(existingEntity, ModelState);
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            this._context.Contact.Update(existingEntity);
            var status = this._context.SaveChanges();
            return Ok(new { status });
        }
    }
